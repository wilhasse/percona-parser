...
TABLE_NAME=${TABLE_NAME:-zlob_fixture}
...
OUT_DIR=${OUT_DIR:-/tmp/ibd-zlob-validate}
...
FILE_PER_TABLE=$("${MYSQL[@]}" -N -B -e "SELECT @@innodb_file_per_table;")
if [ "$FILE_PER_TABLE" != "1" ]; then
  echo "innodb_file_per_table must be ON for ROW_FORMAT=COMPRESSED tests."
  exit 1
fi
...
CREATE TABLE $TABLE_NAME (
  ...
  big_text LONGTEXT,
  big_blob LONGBLOB
) ENGINE=InnoDB ROW_FORMAT=COMPRESSED KEY_BLOCK_SIZE=${KEY_BLOCK_SIZE};
...
if [ "$ROW_FORMAT" != "Compressed" ]; then
  echo "Table row format is not Compressed (got: $ROW_FORMAT)."
  exit 1
fi
